...
% Trains the SVM and makes sure that the embeddings are in the current file
% as well
SVM = trainSVM;
rng('default');
...
% scores to -1 allowing for easier evaluation
    ...
    elseif (sentimentScores(i) <= -1)
        sentimentScores(i) = -1;
  % If sentence is neutral it is passed to the SVM to be classified the
  % sentiment score is allocated as the mean score
    elseif (sentimentScores(i) == 0)
        vector = word2vec(embeddings, sentWords);
        [ ~ , scores ] = predict(SVM, vector );
        sentimentScores(i) = mean(scores(:,1));
  % If the SVM could not classify the sentence then change the NaN to a 0
        if (isnan(sentimentScores(i)))
            sentimentScores(i) = 0;
        end
    end
 % Calculates the number of true and false positive and negatives
    if ((sentimentScores(i) > 0) & (actualScore(i) == 1))
        truePos = truePos + 1;
    elseif ((sentimentScores(i) < 0) & (actualScore(i) == 0))
        trueNeg =trueNeg + 1;
    elseif ((sentimentScores(i) > 0) & (actualScore(i) == 0))
        falsePos = falsePos + 1;
    elseif ((sentimentScores(i) < 0) & (actualScore(i) == 1))
        falseNeg = falseNeg +1;
    end
end
